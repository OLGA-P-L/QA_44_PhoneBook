...
public class RegistrationTests implements BaseApi {
    ...
    public void registrationNegativeTest_ExRes400() throws IOException {
        ...
        softAssert.assertEquals(response.code(),400,"validate response code");
        softAssert.assertEquals(errorMessage.getStatus(),400,"validate status");
        ...
        softAssert.assertTrue(errorMessage.getMessage().toString()
                .contains("password= At least 8 characters; Must contain at least 1 uppercase letter, 1 lowercase letter, and 1 number;"),
                "validate message");
